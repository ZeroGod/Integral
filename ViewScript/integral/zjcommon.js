...
zhongjiu.integral.confirmNew = (function () {
    var defaults = {
        txt: "",//文本内容，和下面一项html相同
        html: "",
        addClass: "",//自定义的样式
        callback: jQuery.noop,//回调函数
        state: "confirmNew" //当state=alert时，为定时器，暂时没用
    };
...
//弹出终端详细信息下拉菜单
zhongjiu.integral.terminalInfo = (function () {
    var defaults = {
        txt: "",
        html: "",
        addClass: "",
    ...
    var initialize = function () {
        var confirm01 = new TerminalInfo(defaults);
        return confirm01;
    };
    return {
        init: function (options) {
...
Class('TerminalInfo', null, {
    TerminalInfo: function (options) {
        jQuery.extend(this, options);
        this.sure = options.sure || jQuery.noop;
        this.creatAlert();
    },
    ...
    bindEvent: function () {
        ...
        this.oSure.click(function () {
            ...
            $('.txtarea').attr("placeholder", content);
            //发送通知标题
            ...
            if (item != "showDiv1") {
                $("#file").val("");
                $('#showImg').html('<img src="/Images/dpmr_2x.png" style="width: 131px; height: 114px;"/>');
            }
            //对应的内容显示
            $('.' + item).css('display', 'block');
            //关闭下拉框
